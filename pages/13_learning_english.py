...
st.set_page_config(page_title="📚 학습 - 영어", page_icon="📚", layout="wide")
with st.sidebar:
...
set_current_page('learning_english')
...
with st.container(border=True):
    ...
    if last_ai_text and st.button("🔊 마지막 AI 답변 듣기"):
        mp3_path = synthesize_to_file(last_ai_text, filename="learn_english_last_ai.mp3")
        if mp3_path:
            st.audio(mp3_path)
        else:
            st.error("Failed to generate audio.")

    prompt = st.chat_input("영어로 질문하거나 연습해보세요...")
    if prompt:
...
with col3:
    render_new_chat_controls(page_key='learning_english', category_name='english')