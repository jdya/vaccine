...
st.set_page_config(page_title="📚 학습 - 역사", page_icon="📚", layout="wide")
with st.sidebar:
...
set_current_page('learning_history')

st.title("📚 학습 - 역사")
st.caption("시대/사건/인물을 중심으로 원인-전개-결과 구조로 이해해요.")
...
with st.container(border=True):
    ...
    prompt = st.chat_input("역사 주제나 인물을 입력하세요… 예: 프랑스 혁명의 원인")
    if prompt:
        ...
        if full_text.strip():
            ...
            cat_row = get_category_by_name('history')
            if cat_row and user:
                save_conversation(
                    user_id=user['id'],
...
with col1:
    if st.button("대화 지우기"):
        ...
        st.rerun()
with col2:
    st.caption("사료의 사실과 관점을 구분하고 오늘날과 연결해 봅시다.")